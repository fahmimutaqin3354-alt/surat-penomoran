...
<div class="space-y-6">
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        <div class="p-4 overflow-x-auto">
            <table id="tableSuratKeluar" class="display w-full text-sm border-collapse">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('surat_keluar.show', $item->id) }}"
                                   class="w-9 h-9 rounded-lg bg-cyan-600 hover:bg-cyan-700 flex items-center justify-center" title="Detail">
                                    <i class="fa-solid fa-eye text-white text-sm"></i>
                                </a>

                                <a href="{{ route('surat_keluar.preview', $item->id) }}"
                                   class="w-9 h-9 rounded-lg bg-indigo-600 hover:bg-indigo-700 flex items-center justify-center" title="Preview File">
                                ...
                                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                                    <button type="button" @click="open = !open"
                                            class="w-9 h-9 rounded-lg bg-violet-600 hover:bg-violet-700 flex items-center justify-center" title="Kirim Surat">
                                ...
                                <form action="{{ route('surat_keluar.destroy', $item->id) }}" method="POST" class="deleteForm">
                                    @csrf
                                    @method('DELETE')
                                    <button class="w-9 h-9 rounded-lg bg-rose-600 hover:bg-rose-700 flex items-center justify-center" title="Hapus">
...
{{-- MODALS SECTION (Diletakkan di luar tabel agar HTML bersih) --}}
...
$(document).ready(function () {
    $('#tableSuratKeluar').DataTable({
        ...
        info: true,
        columnDefs: [
            { orderable: false, targets: [0, 6] }, // Matikan sorting di kolom No dan Aksi
            { className: "text-center", targets: [0, 5, 6] } // Rata tengah No, Status, Aksi
        ],
        language: {
            search: "🔍 Cari :",
