...
{{-- Style khusus untuk mengubah warna ikon kalender --}}
<style>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-slate-400 mt-1">
                Tambahkan data surat keluar baru ke dalam sistem.
            </p>
        </div>

        <a href="{{ route('surat_keluar.index') }}"
           class="px-5 py-3 rounded-xl bg-slate-700 hover:bg-slate-600 text-white font-semibold transition flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="jenis_surat" required
                            ...
                            <option value="Surat Tugas" {{ old('jenis_surat') == 'Surat Tugas' ? 'selected' : '' }}>
                                Surat Tugas
                            </option>
                            <option value="Surat Undangan" {{ old('jenis_surat') == 'Surat Undangan' ? 'selected' : '' }}>
                                Surat Undangan
                            </option>
                            <option value="Surat Pemberitahuan" {{ old('jenis_surat') == 'Surat Pemberitahuan' ? 'selected' : '' }}>
                                Surat Pemberitahuan
                            </option>
                            <option value="Surat Permohonan" {{ old('jenis_surat') == 'Surat Permohonan' ? 'selected' : '' }}>
                                Surat Permohonan
                            </option>
                        </select>
                    </div>
                    ...
                    {{-- Tujuan Instansi (Daftar Instansi dari Database) --}}
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">
                        ...
                        <select name="instansi_id" required
                            ...
                            @foreach ($instansi as $item)
                                <option value="{{ $item->id }}" {{ old('instansi_id') == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama_instansi }} {{ isset($item->kode_instansi) ? '('.$item->kode_instansi.')' : '' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="penandatangan" value="{{ old('penandatangan') }}" required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="jabatan_penandatangan" value="{{ old('jabatan_penandatangan') }}" required
